...
export const Home = () => {
  ...
  const { data: recipes, isValidating } = useSWR<IRecipe[]>(
    `${Variables.URL}/recipes`
  );
    ...
    <>
      <div className="home-container">
        <div className="home-title-inner-container">
          <Header
            ...
            button={{
              ...
              onClick: () => navigation.push("/create-recipe"),
            }}
          />
        </div>
        ...
        <div className="home-input-container">
          <Input
            debounceTime={500}
            placeholder="Search for a recipe by name or by ingredients"
        ...
        <div className="home-inner-container">
          {isValidating ? (
          ...
          ) : recipes && (recipes as any).data.length > 0 ? (
            (recipes as any).data.map((recipe: any) => {
              return (
                <Recipe
                  key={recipe.id}
                  id={recipe.id}
            ...
            <>
              <div style={{ marginTop: "48px" }}>
                <NoResultsSvg width="90%" height="90%" />
                <h2>It looks like you don't have any recipes 🥺</h2>
              </div>
            </>
          )}
        </div>
